...
export default function SubscriptionPage() {
  ...
  const reqPct = data.limits.requestsPerMonth > 0
    ? Math.min(100, (data.usage.requestsThisMonth / data.limits.requestsPerMonth) * 100)
    : 0;
  const userPct = data.limits.users > 0
    ? Math.min(100, (data.usage.users / data.limits.users) * 100)
    : 0;
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-gray-600">Requisições (este mês)</span>
                  <span className="font-semibold text-gray-900">
                    {data.usage.requestsThisMonth} / {data.limits.requestsPerMonth < 99999 ? data.limits.requestsPerMonth : 'ilimitado'}
                  </span>
                </div>
